...
public class Commands {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Your endpoint is: " + uuid);
        Commands c;
        ...
        while (i < args.length) {
            switch (args[i]) {
                ...
                case "-loadcamps":
                    who = getValue(i + 1, args, "-who", "*");
                    String ids = getValue(i + 1, args, "-cids", null);
                ...
                case "-setweights":
                    ...
                    String weights = args[i+3];
                    c.sendSetWeights(who,cid,weights);
                    i = args.length;
                    break;
                case "-getweights":
                    who = getValue(i + 1, args, "-who", "*");
                    cid = getValue(i + 1, args, "-cid", null);
                    c.sendGetWeights(who,cid);
                    i = args.length;
                    break;
                case "-getprice":
                    who = getValue(i + 1, args, "-who", "*");
    ...
    public static String usage() {
        String str = "" +
                ...
                "-getweights [-who bidderid] -cid campid\n" +
                "-listbidders\n" +
                "-listcamps\n" +
                "-loadcamps -who * | bidderid -cids campid\n" +
                "-loaddb filename\n" +
                "-printdb\n" +
                "-setprice [-who * | bidderid] -cid campid -crid creativeid -price nn\n" +
                "-setweights [-who bidderid] -cid \"<crid1>=n,<crid2>=n,<crid2>=n,...\"\n" +
    ...
    public Commands(String host, int pport, int sport, int listen) throws Exception {
        ...
        redisson.setSharedObject(host,6000,6001,listen);
...
//            }
    ...
    public void sendSetWeights(String who, String cid, String weights) {
        SetWeights cmd = new SetWeights(who,cid,weights);
        cmd.from = uuid;
        commands.add(cmd);
    }
