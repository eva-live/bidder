...
public class ElasticLoader {
	...
	private static String redirect = "/callback/target=redirect&exchange=__EXCHANGE__&ad_id=__ADID__&creative_id=__CRID__&cost=__COST__/&lat=__LAT__&lon=__LON__?url=http://__HOST__:8080/contact.html";
	...
	public static void main(String[] args) throws Exception {		
		...
		i = 0;
		while(i < args.length) {
			args[i] = StrSubstitutor.replace(args[i], env);
			switch(args[i]) {
			case "-h": 
		...
		String winURL = "http://" + HOST + "/rtb/win/";
		...
		while(running == true) {
			...
			try {
				...
				if (win && hisBid != null) {
					...
					String theWin = makeWin(map, theBid, rets, wc.doubleValue(),exchange);
	...
	private static void fixWH(Map map) {
	//	System.out.println("Before: " + map);
		List imps = (List)map.get("imp");
		if (imps == null)
			return;
		
		for (int i=0;i<imps.size();i++) {
			Map m = (Map)imps.get(i);
			m.put("instl",0);
		
			Map banner = (Map)m.get("banner");
			if (banner !=  null) {
	...
	private static String makeWin(Map bid, Map theBid, Map r, double dcost, String exchange) {
		String str = winnah;
		String lat = "NA";
		String lon = "NA";
		String cost = "";
